...
class StepOneTrainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, idxs in enumerate(self.data_sampler):
            ...
            class_protos_x, class_protos_len, class_protos_y = get_samples(class_samples, self.device,
    ...
    def _valid_epoch(self, epoch):
        ...
        if self.mnt_mode != 'off' and res['GZSL_acc_hm'] >= self.mnt_best:
            with open(
                    './data/ver1/{}/protos_{}.pkl'.format(self.config['dataset']['name'], self.config['encoder_type']),
                    'wb') as f:
...
class StepTwoTrainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (unseen_classes, unseen_query, seen_classes, seen_query) in enumerate(self.data_sampler):
            ...
            unseen_protos_x, unseen_protos_len, unseen_protos_y = get_samples(unseen_class_samples, self.device,
                                                                              self.config['encoder_type'])
            ...
            unseen_querys_x, unseen_querys_len, unseen_querys_y = get_samples(unseen_query_samples, self.device,
                                                                              self.config['encoder_type'])
            seen_query_samples = [self.dataset.train_seen[ids] for ids in seen_query]
            seen_querys_x, seen_querys_len, seen_querys_y = get_samples(seen_query_samples, self.device,
                                                                        self.config['encoder_type'])
            ...
            unseen_querys = self.model(unseen_querys_x, unseen_querys_len, protos, semantic_components, n_seen,
                                       'sample_adapt')
            seen_querys = self.model(seen_querys_x, seen_querys_len, protos, semantic_components, n_seen,
                                     'sample_adapt')
